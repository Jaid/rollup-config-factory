...
export const runTest = async (testContext) => {
    ...
    if (fixtureConfigFileExists) {
        fixtureConfig = await import(pathToFileURL(fixtureConfigFile).toString());
    }
    const context = {
        testContext,
    ...
    const outputMeta = async (outputId, value) => {
    ...
    try {
        ...
        if (fixtureConfig.checkExport) {
            ...
            const exportValue = await import(pathToFileURL(mainFile).toString());
